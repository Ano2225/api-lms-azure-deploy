...
exports.createLayout = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (isTypeExist) {
            return next(new ErrorHandler_1.default(`${type} existe déjà`, 400));
        }
        if (type === "Banner") {
        ...
        res.status(200).json({
            success: true,
            message: "Layout créé avec succès",
        });
    }
    catch (error) {
...
exports.editLayout = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: "Layout mis à jour avec succès",
        });
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 500));

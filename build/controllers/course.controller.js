...
exports.getCourseByUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!courseExists) {
            return next(new ErrorHandler_1.default("Vous n'êtes pas autorisé à accéder à ce cours", 404));
        }
...
exports.addQuestion = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        await notification_Model_1.default.create({
            user: req.user?._id,
            title: "Nouvelle question reçue",
            message: `Vous avez une nouvelle question dans ${couseContent.title}`,
        });
...
exports.addAnwser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (req.user?._id === question.user._id) {
            ...
            await notification_Model_1.default.create({
                user: req.user?._id,
                title: "Nouvelle réponse à une question reçue",
                message: `Vous avez une nouvelle réponse à une question dans ${couseContent.title}`,
            });
        }
        else {
            const data = {
...
exports.addReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        await notification_Model_1.default.create({
            ...
            message: `${req.user?.name} a donné un avis sur ${course?.name}`,
        });
        res.status(200).json({
            success: true,
            course,
...
exports.addReplyToReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const course = await course_model_1.default.findById(courseId);
        if (!course) {
            return next(new ErrorHandler_1.default("Cours non trouvé", 404));
        }
        const review = course?.reviews?.find((rev) => rev._id.toString() === reviewId);
        if (!review) {
            return next(new ErrorHandler_1.default("Avis non trouvé", 404));
        }
        const replyData = {
            user: req.user,
            comment,
...
exports.generateVideoUrl = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    ...
    // Ajout de logs pour vérifier les entrées
    ...
    try {
        ...
        console.log("Response from VdoCipher:", response.data);
